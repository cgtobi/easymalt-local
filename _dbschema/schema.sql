CREATE TABLE `accounts` (
  ...
  `routing_number` varchar(5) NOT NULL DEFAULT '',
...
CREATE TABLE `post_processing` (
  ...
  `tags` set('Reimbursable','Reimbursed','Tax Related') DEFAULT NULL,
...
CREATE TABLE `transactions` (
  ...
  `type` enum('ATM','DEBIT','CREDIT','OTHER') DEFAULT NULL,
...
CREATE VIEW `v_transactions_reports`
AS SELECT
      t.id AS `id`,
      t.date AS `date`,
