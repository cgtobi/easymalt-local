...
class DB
{
    ...
    public static function execute($q, $args = array(), $retryOnError=TRUE) {
        ...
        if (DEBUGSQL) {
            ...
            $date_prefix = "[" . substr(date('c'), 0, 19) . (DEBUG ? ".".round($ms[1]/10) : "") . "]";
            echo "$date_prefix ";
            foreach ($args as $k => $v) {
                echo "SET @$k = '$v'; ";
            }
        ...
        try {
            $stmt->execute();
            if (DEBUGSQL) {
                echo "$date_prefix  Query execution completed.\n";
            }
            return $stmt;
        } catch (\PDOException $e) {
            ...
            if ($retryOnError) {
                if ($error_code == 1205) {
                    sleep(1);
                    LogDatabase::logAndPersist("Will retry query $q following a 'Lock wait timeout exceeded' error ($error_code).");
                ...
                if ($error_code == 2013 || $error_code == 2006 || $error_code == 2055) {
                    sleep(5);
                    DB::connect();
                    LogDatabase::logAndPersist("Will retry query $q following a 'Lost connection to DB' error ($error_code).");
